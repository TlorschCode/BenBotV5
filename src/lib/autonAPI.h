...
// #include "robotAPI.h"
// namespace autonAPI {
// inline void PID_Controller::updatePID(Vec2 &_target) {
...
// 	}
// 	prev_pRot = pRot.val;
// 	leftSpeed = PID_pos.x + PID_pos.y; // FIXME: Add rot into leftSpeed and rightSpeed cals, check math
...
// 	}
// }
// inline void PID_Controller::updateOdom(robotAPI::Robot *robot) { // FIXME
// 	float allRotPrev = 0;
// 	robotAPI::Robot dereffedRobot = *robot;
// 	uint32_t now = pros::millis();
// 	dereffedRobot.heading.setDegrees(truncate(dereffedRobot.inertial.get_rotation()));
// 	float leftMotorsPos = (dereffedRobot.drivetrain.wheels.at(0).rawMotor.get_raw_position(&now) + dereffedRobot.drivetrain.wheels.at(1).rawMotor.get_raw_position(&now)) / 2;
// 	float rightMotorsPos = (dereffedRobot.drivetrain.wheels.at(2).rawMotor.get_raw_position(&now) + dereffedRobot.drivetrain.wheels.at(3).rawMotor.get_raw_position(&now)) / 2;
...
namespace robotAPI { class Robot; }  // ✅ forward declare only
